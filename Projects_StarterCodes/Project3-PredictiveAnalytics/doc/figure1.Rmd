...
```{r}
load("../output/information_table.csv")
...
dist_histogram_by_emotion <- function(emotion_idx, pt1, pt2, vertical = T){
    ...
    emotion <- as.character(emotion_table[emotion_table$idx == emotion_idx, 'emotion_cat'])
...
dist_histogram <- function(pt1, pt2, vertical = T){
  lapply(emotion_table$idx, dist_histogram_by_emotion, pt1, pt2, vertical)
}
...
display_fid_pt <- function(identity){
    ...
    emotions <-  info[info$identity == identity, 'emotion_cat']
    Image_list_sub <- lapply(info$image.path[indices], EBImage::readImage)
