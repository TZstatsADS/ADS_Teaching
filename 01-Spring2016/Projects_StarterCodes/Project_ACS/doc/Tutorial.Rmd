...
library(dplyr)
library(repmis)
# dropbox data link: https://www.dropbox.com/s/09uc0xg0ujkfa44/ss13husa.csv?dl=0
acs13husa <- source_DropboxData(file = "ss13husa.csv", 
             key = "09uc0xg0ujkfa44", sep = ",", header = TRUE)
```
...
```{r, message=F}
ST.anno=read.csv("../data/statenames.csv", header=T)
ST.anno=mutate(ST.anno, STabbr=abbr, STname=name)
acs13husa = left_join(acs13husa, ST.anno, by = c("ST" = "code"))
