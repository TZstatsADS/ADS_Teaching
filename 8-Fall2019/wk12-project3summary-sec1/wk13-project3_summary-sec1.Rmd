---
title: "Summary of Project 3 (Section 1)"
...
```{r, echo=FALSE}
# run Results.R first
submitTime <- c(60, 1, 60, 0, 5, 5, 0, 60, 34)
submitTime_label <- c("> 1 day", "6:51PM", "> 1 day", "6:49PM", "6:55 PM", "6:55PM", "6:39PM", "> 1 day", "7:24PM")
bar_time <- barplot(submitTime, main="Submission Time: Minutues from 6:50 PM",
        xlab="Team", ylab="Minutes", ylim=c(0, 80))
...
```{r, echo=FALSE}
# run Results.R first
RunningTime <- c(10, 324, 749, 351, 165, 500, 833, 116, 166)
...
Advanced models: CNN (1, 3, 5, 7), logistic regression (2, 4, 8), SVM (6, 9).

```{r, echo=FALSE}
# run Results.R first
RunningTime <- c(15, 420, 95, 28, 14, 12, 342, 11, 113)
log_RunningTime <- log(RunningTime)
...
+ labs(title="Baseline Model Accuracy", x="Team", y="Accuracy")
...
+theme(axis.line = element_line(colour = "black"),
      ...
      strip.text.y = element_text(size = 13, face="bold"),
      panel.background = element_blank())
+ labs(title="Advanced Model Accuracy", x="Team", y="Accuracy")
+ scale_colour_brewer(palette = "Dark2", guide = FALSE)
+ scale_fill_brewer(palette = "Pastel2", labels=type_names, guide = guide_legend(title = "Type"))
+ geom_vline(xintercept = seq(1.5, 13.5, 1), linetype = "longdash", colour="lightgrey")) 
```
